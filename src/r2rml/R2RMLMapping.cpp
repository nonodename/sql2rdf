...
R2RMLMapping::R2RMLMapping(R2RMLMapping &&other) noexcept
    : triplesMaps(std::move(other.triplesMaps)), serdEnvironment(std::exchange(other.serdEnvironment, nullptr)) {
}
...
bool R2RMLMapping::isValid() const {
	return std::all_of(triplesMaps.begin(), triplesMaps.end(),
	                   [](const std::unique_ptr<TriplesMap> &tm) { return tm && tm->isValid(); });
}

bool R2RMLMapping::isValidInsideOut() const {
	return std::all_of(triplesMaps.begin(), triplesMaps.end(),
	                   [](const std::unique_ptr<TriplesMap> &tm) { return tm && tm->isValidInsideOut(); });
}
