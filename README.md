...
| `test_runner` | executable | none | Test suite using [Catch2](https://github.com/catchorg/Catch2) v2 for C++11 compatiblity. All tests run against a mock SQL backend — no DuckDB required. Gated by `SQL2RDF_BUILD_TESTS` (default: ON when building standalone, OFF when consumed via `FetchContent`). |
