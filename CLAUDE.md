...
3. **`SQL2RDF++` CLI** (`src/main.cpp`, `src/DuckDBConnection.*`, `src/TypeCatalogLoader.*`) — the only code that touches DuckDB. `DuckDBConnection.h` deliberately lives in `src/`, not `include/`. `TypeCatalogLoader` sits in the same layer because it issues backend-specific catalog SQL (`information_schema` + `DESCRIBE`) to fill a `sparql2sql::TypeCatalog`; it is shared by the CLI, the benchmark harness and `sparql2sql_duckdb_tests` via the `sql2rdf_duckdb` static library.
