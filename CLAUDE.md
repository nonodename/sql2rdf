...
Test fixtures: R2RML Turtle mappings in `tests/sourceR2RML/`, YARRRML equivalents plus feature/error fixtures in `tests/sourceYARRRML/`, SPARQL query fixtures (valid queries and `invalid_*.rq` error cases) in `tests/sourceSPARQL/`, SPARQL-to-SQL translator query fixtures in `tests/sourceSPARQL2SQL/` (translated against the R2RML fixtures above, mainly `example_emp_dept.ttl` and a few dedicated toy mappings prefixed `sparql2sql_*.ttl`). Named graphs have three purpose-built mappings worth knowing about: `sparql2sql_graphs.ttl` covers every graph-set shape (constant/template graph maps, an ungraphed predicate-object map, `rr:defaultGraph` as a set member, and `rr:class` under a subject-level graph); `sparql2sql_graph_path.ttl` puts each edge of a 1→2→3 chain in a *different* named graph, which is what makes a wrongly-cross-graph property-path hop observable; and `default_graph_union.ttl` covers the forward direction's `rr:defaultGraph`-plus-named-graph union. The directories are passed to the test binary as compile definitions `SOURCE_R2RML_DIR` / `SOURCE_YARRRML_DIR` / `SOURCE_SPARQL_DIR` / `SOURCE_SPARQL2SQL_DIR`, so tests reference fixtures by bare filename. R2RML/YARRRML fixtures are based on the examples in the W3C R2RML spec.
...
**`sql2rdf_sparql2sql`** (`src/sparql2sql/`, headers in `include/sparql2sql/`, namespace `sparql2sql::`) is that future task: it translates a parsed SPARQL query (`sparql::ast::Query`) against a parsed R2RML mapping (`r2rml::R2RMLMapping`) into a SQL string for a given `SqlDialect`, by using the mapping's `TriplesMap`/`PredicateObjectMap`/`TermMap` structure in reverse (enumerate which mapping sources could produce a triple matching a given triple pattern, then compose per-pattern SQL relations via the SPARQL algebra — AND→inner join, OPTIONAL→left outer join, UNION→schema-extending union, MINUS→anti-join). It depends on **both** `sql2rdf_r2rml` and `sql2rdf_sparql`; neither of those may depend back on it, and it has no DuckDB/yaml-cpp dependency of its own (so it links safely into the DuckDB-free `test_runner`). Entry point: `sparql2sql::translateQuery()` (`include/sparql2sql/Translator.h`). Only SELECT/ASK query forms are supported. Named graphs are supported in reverse: `rr:graph`/`rr:graphMap` become a fourth term position, under **strict RDF-dataset semantics** (a graph-less query sees only the default graph). `FROM`/`FROM NAMED` restrict the active dataset (§13.2), including the two rules that surprise people: `FROM` replaces the default graph, and `FROM NAMED` alone leaves it empty. Property paths are desugared into that same algebra by `translatePath()` (`src/sparql2sql/PropertyPathTranslator.cpp`) per SPARQL 1.1 §18.1.7 — `^`, `/`, `|`, `?` and negated property sets all work; the arbitrary-length operators `*`/`+` work through recursive CTE SQL. Every SPARQL variable is represented as a plain SQL VARCHAR of the term's lexical form, with a term-kind/datatype/language dimension derived from the mapping alongside it (`include/sparql2sql/TermInfo.h`; combined across candidate arms by a meet). That dimension is resolved **statically wherever the mapping determines it** and folded into a constant, so a well-typed mapping generates the same SQL it always did; where the mapping cannot determine it, the dimension is carried into the generated SQL as a companion `d_<var>` **type-tag column** (`include/sparql2sql/TagSql.h`) and evaluated per row, which is what makes the term-kind builtins, RDF term equality, SPARQL value comparison and §15.1 ordering work over predicates whose candidate term maps disagree. Tag columns are materialised only on demand, by the two passes in `include/sparql2sql/TagDemand.h` — join keys before `optimize()`, expressions after it so filter pushdown gets first refusal at resolving a predicate per union arm. See `doc/api.md`'s "Supported SPARQL subset / Known limitations" for the full, current list. The CLI exposes it via `-T <file.rq> [--dialect <name>]`, paired with the mapping-file positional argument; if the database-file positional is also given, the translated SQL is additionally executed and its result rows printed (see `-h`).
